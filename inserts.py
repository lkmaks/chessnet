...
def add_real_game(white_id, black_id, result, time_left1, time_left2, date, time_control, control_type, tournament_id, conn):
    # result: 0 - white win, 1 - draw, 2 - black win
    with conn.cursor() as cursor:
        query = "insert into realgame (tournament_id, game_date, time_control, game_result, control_type) values ({}, {}, '{}', {}, '{}')".format(
        ...
        # c3 = get_attr('realplayer', black_id, 'country', conn)
        typo = 'world'
        # if c1 == c2 and c2 == c3:
        #     typo = 'country'
        # recalculate rating
        ...
        if result == 0:
            ...
            query = 'update realplayer set wins_as_white = {} where id = {}'.format(str(wins_as_white + 1), str(white_id))
            cursor.execute(query)
            query = 'update realplayer set losses_as_black = {} where id = {}'.format(str(losses_as_black + 1), str(black_id))
            cursor.execute(query)
        elif result == 2:
            ...
            query = 'update realplayer set losses_as_white = {} where id = {}'.format(str(losses_as_white + 1), str(white_id))
            cursor.execute(query)
            query = 'update realplayer set wins_as_black = {} where id = {}'.format(str(wins_as_black + 1), str(black_id))
            cursor.execute(query)
        elif result == 1:
            ...
            query = 'update realplayer set draws_as_white = {} where id = {}'.format(str(draws_as_white + 1), str(white_id))
            cursor.execute(query)
            query = 'update realplayer set draws_as_black = {} where id = {}'.format(str(draws_as_black + 1), str(black_id))
            cursor.execute(query)
